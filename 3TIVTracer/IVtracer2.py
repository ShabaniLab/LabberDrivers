    ...
    def _perform_point_by_point_acquisition(self, with_li: bool):
        ...
        if with_li:
            ...
            self._didv_trace1 = (
                sigma1
                / (2 * cs.e ** 2 / cs.h)
                / (1 - (sigma1 + sigma2) * self.getValue("Inline resistance front")
                    - sigma1 * self.getValue("Inline resistance 1"))
                )
            self._didv_trace2 = (
                sigma2
                / (2 * cs.e ** 2 / cs.h)
                / (1 - (sigma1 + sigma2) * self.getValue("Inline resistance front")
                    - sigma2 * self.getValue("Inline resistance 2"))
                )
            self._realbias1 = (
                set_points / self.getValue("DC Voltage divider")
                - offset
                - (dmm_vals1 + dmm_vals2) * self.getValue("Inline resistance front")
            ...
            self._realbias2 = (
                ...
                - offset
                - (dmm_vals1 + dmm_vals2) * self.getValue("Inline resistance front")
                - dmm_vals2 * self.getValue("Inline resistance 2")
                )

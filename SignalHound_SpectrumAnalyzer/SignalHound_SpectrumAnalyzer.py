...
class Driver(InstrumentDriver.InstrumentWorker):
	...
	def performOpen(self,options={}):
		...
		self.sighound = ctypes.CDLL(r"C:\Program Files\Signal Hound\Spike\sa_api.dll")
		...
		if quant.name=="Signal" or quant.name=="Signal - Zero span":
			if(self.isFirstCall(options)) :
				if self.initiateFlag:
					self.sighound.saInitiate(self.handle,
					                         ctypes.c_int(0),
											 ctypes.c_int(0))
					self.sighound.saQuerySweepInfo(self.handle,
				...
				self.sighound.saGetSweep_32f(self.handle,
			...
			if quant.name=="Signal":
				return quant.getTraceDict(self.min,
										  x0=self.startFreq.value,
										  dx=self.binSize.value)
			else:

...
class Driver(InstrumentDriver.InstrumentWorker):
    ...
    def performOpen(self, options={}):
        ...
        self.sighound = ctypes.CDLL(os.path.join(driver_dir, r"sa_api.dll"))
        ...
        if self.sighound.saOpenDevice(ctypes.byref(self.handle)) != 0:
            ...
            self.sighound.saConfigAcquisition(
                self.handle, ctypes.c_int(1), ctypes.c_int(0)
            )
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        if quant.name == "Center frequency" or quant.name == "Span":
            self.sighound.saConfigCenterSpan(
                self.handle,
                ctypes.c_double(self.getValue("Center frequency")),
                ctypes.c_double(self.getValue("Span")),
            )
        if quant.name == "Bandwidth":
            self.sighound.saConfigSweepCoupling(
                self.handle,
                ctypes.c_double(self.getValue("Bandwidth")),
                ctypes.c_double(self.getValue("Bandwidth")),
                ctypes.c_int(1),
            )
        if quant.name == "Input Power Level":
            self.sighound.saConfigLevel(
                self.handle, ctypes.c_double(self.getValue("Input Power Level"))
            )
        ...
        if quant.name == "Signal" or quant.name == "Signal - Zero span":
            if self.isFirstCall(options):
                if self.initiateFlag:
                    self.sighound.saInitiate(
                        self.handle, ctypes.c_int(0), ctypes.c_int(0)
                    )
                    self.sighound.saQuerySweepInfo(
                        self.handle,
                        ctypes.byref(self.sweepLen),
                        ctypes.byref(self.startFreq),
                        ctypes.byref(self.binSize),
                    )
                ...
                self.sighound.saGetSweep_32f(
                    self.handle,
                    self.min.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                    self.max.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
                )
            if quant.name == "Signal":
                return quant.getTraceDict(
                    self.min, x0=self.startFreq.value, dx=self.binSize.value
                )
            else:

...
from VICurveTracer_2DMM import VoltMeter
...
class Driver(VoltMeter):
    ...
    def prepare_acquisition(self, points):
        ...
        # Timeout in ms
        self._timeout = points * 2.0 / float(self._rsc.query("VOLT:DC:NPLC?")) * 1000
    ...
    def arm_device(self):
        ...
        # storage.
        rsc.write(
            ":DISP:ENAB 0;:TRAC:CLE;:STAT:MEAS:ENAB 512;*SRE 1;:DATA:FEED:CONT NEXT;:INIT"
        )
    ...
    def wait_for_data_ready(self):
        ...
        """
        self._rsc.wait_for_srq(timeout=self._timeout)
    ...
    def retrieve_data(self):
        ...
        self._rsc.read_bytes(2)
        block = self._rsc.read_bytes(self._points * 8 + 1)

        return util.from_binary_block(
            block, 0, self._points * 8, "d", False, np.ndarray
        )

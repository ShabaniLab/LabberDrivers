    ...
    def performGetValue(self, quant, options={}):
        ...
        if quant.name in ('S11 - Enabled', 'S21 - Enabled', 'S12 - Enabled',
            ...
            if quant.name in self.dMeasParam:
                ...
                # Get frequency
                self.write(':CALC:SEL:DATA:XAX?', bCheckError=False)
                xData = self.read(ignore_termination=True)
                # if bWaitTrace and not bAverage:
                #     self.writeAndLog(':INIT:CONT ON;')
                # strip header to find # of points
                i0 = xData.find(b'#')
                nDig = int(xData[i0+1:i0+2])
                ...
                # logX = False  #(sweepType == 'Log')
                # lorX = False  # (sweepType == 'Lorentzian')

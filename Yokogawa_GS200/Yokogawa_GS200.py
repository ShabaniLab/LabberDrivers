...
class Driver(VISA_Driver):
    ...
    def performArm(quant_names, options={}):
        """Nothing to arm since the setting should have prepared everything.
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        if quant.name in ("Voltage", "Current"):
            ...
            if abs(value) > dMax:
                ...
                raise Exception(
                    "New value (%.6g) is out of range (max = %.6g)" % (value, dMax)
                )
            # calculate actual value based on smallest step size
            value = dStep * np.round(value / dStep)
            # check if continuous sweep mode or output off, if not call generic driver
            if sweepRate == 0.0 or (not self.getValue("Output")):
                return VISA_Driver.performSetValue(
                    self, quant, value, 0.0, options=options
                )
            ...
            # In hardware triggering mode ensure the program start signal is
            # properly routed
            hardware_trig = self.isHardwareTrig(options)
            if hardware_trig:
                VISA_Driver.writeAndLog(self, ":ROUT:BNCO TRIG")
            ...
            sCmd = (
                ...
                + ":PROG:EDIT:END;"
                + ":PROG:RUN"
                if not hardware_trig
                else ""
            )
            self.is_sweeping = not hardware_trig
            ...
            if quant.name == "Start Program":
                self.is_sweeping = True
            return VISA_Driver.performSetValue(
                self, quant, value, sweepRate, options=options
            )
    ...
    def getMaxValueAndSmallestStep(self):
        ...
        if func == "Voltage":
            ...
            dMax = {"200 mA": 0.2, "100 mA": 0.12, "10 mA": 0.012, "1 mA": 0.0012}
